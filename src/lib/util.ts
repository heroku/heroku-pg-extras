...
export async function ensurePGStatStatement(connectionDetails: ConnectionDetailsWithAttachment): Promise<void> {
...
export function ensureEssentialTierPlan(db: AddOnWithRelatedData): void {
  if (db.plan?.name.match(/(dev|basic|essential-\d+)$/)) {
    throw new Error('This operation is not supported by Essential-tier databases.')
  }
}

export function essentialNumPlan(db: AddOnWithRelatedData): boolean {
...
export async function newTotalExecTimeField(connectionDetails: ConnectionDetailsWithAttachment): Promise<boolean> {
...
export async function newBlkTimeFields(connectionDetails: ConnectionDetailsWithAttachment): Promise<boolean> {
