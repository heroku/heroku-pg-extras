...
const PgTableIndexesSize = require('../../../dist/commands/pg/table-indexes-size').default
...
describe('PgTableIndexesSize', function () {
  ...
  before(function () {
    command = new PgTableIndexesSize()
  })
  ...
  describe('Command Class', function () {
    it('should have correct static description', function () {
      expect(PgTableIndexesSize.description).to.equal('show the total size of all the indexes on each table, descending by size')
    })

    it('should have correct static args', function () {
      expect(PgTableIndexesSize.args).to.have.property('database')
      expect(PgTableIndexesSize.args.database.description).to.equal('database name')
    })

    it('should have correct static flags', function () {
      expect(PgTableIndexesSize.flags).to.have.property('app')
      expect(PgTableIndexesSize.flags).to.have.property('remote')
      expect(PgTableIndexesSize.flags.app.required).to.be.true
    })
  })

  describe('run()', function () {
    it('should execute table indexes size query successfully', async function () {
      ...
      expect(query).to.include('c.relname AS table')
      expect(query).to.include('pg_size_pretty(pg_indexes_size(c.oid)) AS index_size')
      ...
      expect(query).to.include('AND c.relkind=\'r\'')
      expect(query).to.include('ORDER BY pg_indexes_size(c.oid) DESC')
    })
    ...
    it('should only include regular tables', async function () {
      const args = createTestArgs()
      const flags = createTestFlags()
      ...
      // Verify the query only includes regular tables (relkind='r')
      const query = utils.pg.psql.exec.firstCall.args[1]
      expect(query).to.include('c.relkind=\'r\'')
    })

    it('should order results by index size descending', async function () {
      const args = createTestArgs()
      const flags = createTestFlags()
      ...
      // Verify the query orders by index size descending
      const query = utils.pg.psql.exec.firstCall.args[1]
      expect(query).to.include('ORDER BY pg_indexes_size(c.oid) DESC')
    })
  })
