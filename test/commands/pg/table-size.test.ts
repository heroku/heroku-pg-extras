...
import PgTableSize from '../../../src/commands/pg/table-size'
...
describe('pg:table-size', function () {
  ...
  context('when the --app flag is specified', function () {
    context('when table size query executes successfully', function () {
      it('shows table size information', async function () {
        ...
        require('@heroku/heroku-cli-util').utils.pg.psql.exec = () => Promise.resolve('users | 50 MB\nposts | 25 MB\ncomments | 10 MB')

        try {
          await runCommand(PgTableSize, ['--app=my-app'])

          expect(stripAnsi(stdout.output)).to.include('users | 50 MB')
          expect(stripAnsi(stdout.output)).to.include('posts | 25 MB')
          expect(stripAnsi(stdout.output)).to.include('comments | 10 MB')
    ...
    context('when database connection fails', function () {
      it('shows error message', async function () {
        ...
        try {
          await expectRejection(runCommand(PgTableSize, ['--app=my-app']), 'Database connection failed')
        } finally {
          require('@heroku/heroku-cli-util').utils.pg.fetcher.database = originalFetcher
        }
      })
    ...
    context('when query execution fails', function () {
      it('shows error message', async function () {
        ...
        try {
          await expectRejection(runCommand(PgTableSize, ['--app=my-app']), 'Query execution failed')
        } finally {
          // Restore original functions
          require('@heroku/heroku-cli-util').utils.pg.fetcher.database = originalFetcher
          require('@heroku/heroku-cli-util').utils.pg.psql.exec = originalExec
    ...
    context('when database argument is specified', function () {
      it('executes query against specified database', async function () {
        ...
        require('@heroku/heroku-cli-util').utils.pg.fetcher.database = () => Promise.resolve(mockDbConnection)
        require('@heroku/heroku-cli-util').utils.pg.psql.exec = () => Promise.resolve('custom_table | 100 MB')

        try {
          await runCommand(PgTableSize, ['--app=my-app', 'custom-db'])

          expect(stripAnsi(stdout.output)).to.include('custom_table | 100 MB')
          expect(stderr.output).to.equal('')
        } finally {
          // Restore original functions
          require('@heroku/heroku-cli-util').utils.pg.fetcher.database = originalFetcher
